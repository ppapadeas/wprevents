...
class CustomSerializer(Serializer):
  ...
  def to_csv(self, data, options=None):
    ...
    writer = CSVUnicodeWriter(raw_data, delimiter=';', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    ...
    for category in data:
      if category == 'objects' and len(data[category]) > 0:
        ...
        for item in data[category]:
          ...
          available_keys += [key for key in flatitem.keys() if key not in available_keys]
        ...
        for item in data[category]:
          flatitem = flatten_dict(item)
          writer.writerow(map(lambda x: flatitem.get(x), available_keys))
